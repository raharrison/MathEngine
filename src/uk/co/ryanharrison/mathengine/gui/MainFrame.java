...
public class MainFrame extends JFrame
{
	...
	/** Panel to house the input and output areas */
	 ...
	 * displays the result in the output {@link TextPane}
	 * 
	...
	private void evaluateAndDisplayResult(String expression)
	{
		try
		{
			// Compile the expression into an expression tree
			evaluator.compileTree(expression);

			// Construct a message containing the input which appears before the
			// output
			...
			output.getDocument().insertString(output.getDocument().getLength(), inputTreeMessage,
					null);
